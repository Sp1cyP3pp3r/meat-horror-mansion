...
func _ready():
	if not effected_game_object:
		# Если цель дебаффа не назначена, будет назначен родитель
		# If target of an effect wasn't assigned, it will assign its parent
		effected_game_object = get_parent()
	
	if not effected_game_object.stats:
		return
	
	apply_effects([])
	apply_predesigned_effect()

func _exit_tree():
	discard_effects()

func apply_predesigned_effect():
	pass
	...
	for property in properties_array:
		...
		if ( property_name in applied_properties ) or is_apply_all_properties:
			if property_name in effected_game_object.stats:
				...
				effected_game_object.stats[property_name] = final_value
